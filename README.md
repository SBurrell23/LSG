...
- **Key** and **Type** default to random. Key is a dropdown of every major and minor key. Type opens a gallery of illustrated tiles grouped by time signature: 4/4 (Ballad, Medium, Rock Ballad, Blues, Boogie-Woogie, Gospel, Bossa Nova, Medium Swing, Medium-Up Swing, Latin, Funk, Reggae, Stride), 3/4 (Waltz, Slow Waltz, Jazz Waltz, Minuet, Country Waltz), 2/4 (Samba, Ragtime, Polka, March, Tango, Habanera) and 6/8 (6/8 Ballad, Jig, Tarantella). Hover a tile (or tap its ⓘ on touch screens) to read what makes that type tick.
- **Play along** plays the melody with the chords as accompaniment (`Space` toggles play/pause); the note being played is highlighted on the sheet. The ♩= button opens a tempo slider (50–250 BPM).
...
- `js/tune.js` – the song-type table (time signature, tempo range, description, rhythm profile and harmony profile per type), key/form/title selection, and the ABC writer.
- **Types** shape the music through two profiles. The *rhythm profile* multiplies the weight of rhythm cells by tag (long notes, eighths, sixteenths, triplets, dotted, syncopated, rests, habanera), so a Ballad leans on long notes and a Bossa Nova on off-beat figures at the same level. The *harmony profile* can force triads (Country Waltz, Polka), dominant 7ths on I and IV (Blues, Funk), slow the chord rate (Reggae), pick the fixed 12-bar blues form, and scale each decoration (secondary dominants, borrowed chords, passing diminished, tritone subs, slash chords, sus). Blues-based types also add blue notes to the melody's fill scale.
- `js/icons.js` – the line icons for the type gallery.
